...
public class AliPay {
    ...
    private static final int ALI_PAY_SUCCESS = 100;

    public static void pay(Activity activity, String orderInfo, PayResultListener listener) {
        Thread thread = new Thread(new RunnableImpl(activity, orderInfo, listener));
        thread.start();
    }
    ...
    public static class RunnableImpl implements Runnable {
        ...
        public RunnableImpl(Activity activity, String orderInfo, PayResultListener listener) {
            mActivity = new WeakReference<>(activity).get();
            mOrderInfo = orderInfo;
            mHandler = new ResultHandler(listener);
        }

        @Override
        public void run() {
            mHandler.removeMessages(ALI_PAY_SUCCESS);
    ...
    public static class ResultHandler extends Handler {
        ...
        public ResultHandler(PayResultListener listener) {
            super();
        ...
        public void handleMessage(Message msg) {
            ...
            if (msg.what != ALI_PAY_SUCCESS) return;
